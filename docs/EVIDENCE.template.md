...
| SmolVLA rows | {{ ", ".join(m.replace("policy_only", "policy-only").replace("policy_", "") + f" {load('seeds_smolvla_' + m + '_test.json')['successes']}/10" for m in ("policy_only", "policy_retry", "policy_fallback") if load("seeds_smolvla_" + m + "_test.json")) or "pending" }} | `eval/run_seeds.py --policy smolvla` | results/seeds_smolvla_*_test.json |
...
| Heatmap seeds × axis, ACT | {{ ("hardest axis " + load("heatmap_act.json")["hardest_axis"]) if load("heatmap_act.json") else "not run: ACT is scored per skill and in the full-task table instead" }} | `eval/heatmap.py --policy act` | results/heatmap_act.json / .png |
...
| Per-skill ACT policy-only (50k steps) | {{ skill_rows_50k() }} | `lerobot-train … --steps=50000 --output_dir=outputs_50k/act_<skill>`; `python -m eval.skill_eval --act-root outputs_50k --tag act_50k` | `results/skill_eval_act_*50k.json` |
| Per-skill SmolVLA policy-only | {{ smolvla_row() }} | `python -m eval.skill_eval --kind smolvla --smolvla-path <Prashant-77/thali_smolvla> --tag smolvla_5k` | `results/skill_eval_smolvla_5k.json` |
...
| Follow-ups and corrections | {{ followups_row() }} | `python -m eval.followups` | `results/followups.json` |
| Clear the table | {{ clear_row() }} | `python -m eval.clear_table --seeds 10 --split test` | `results/clear_table.json` |
