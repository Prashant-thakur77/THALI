...
| Table-state anomaly check | abs(frame − reset reference) crop, resnet18: image AUROC **0.938** · 61/75 disturbances flagged with 5/60 false alarms (spill 6/15, tipped mug 15/15, knocked plate 14/15, fallen bottle 15/15, dropped cutlery 11/15) · at 10% false alarms 63/75 · IR p50 CPU 75.32 ms — ablation full frame, wide_resnet50: AUROC 0.762, 38/60 false alarms — ablation table crop, resnet18: AUROC 0.78, 42/60 false alarms | `python -m anomaly.make_data; .venv-anomalib/bin/python -m anomaly.train_patchcore; python -m anomaly.check --score` | `results/anomaly.json` |
