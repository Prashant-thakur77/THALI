...
| Per-skill SmolVLA policy-only (newest checkpoint) | step 9500: open_drawer **12/20** · pick_place_fork **0/20** (median 30.9 cm from zone) · pick_place_plate **0/20** (median 18.71 cm from zone) · pick_place_mug **0/20** (median 24.47 cm from zone) · handoff_spoon **0/20** (median 39.45 cm from zone) · hold_mug **4/20** · pour **0/20** | `python -m eval.skill_eval --kind smolvla --smolvla-path <Prashant-77/thali_smolvla> --tag smolvla_5k` | `results/skill_eval_smolvla_5k.json` |
