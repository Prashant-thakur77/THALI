...
| Follow-ups and corrections | **19/20** corrections resolved to the expected plan; 14/16 resolved plans verifier-approved (the rest are refused with a reason, e.g. the other arm cannot reach) | `python -m eval.followups` | `results/followups.json` |
