...
|---|---|---|---|
| Expert full task, test split (10 seeds) | 7/10 | `eval/run_seeds.py --policy expert --split test` | results/seeds_expert_expert_test.json |
...
| Per-skill ACT policy-only (50k steps) | pick_place_fork **16/20** (median 1.04 cm from zone) · pick_place_plate **15/20** (median 1.85 cm from zone) | `lerobot-train … --steps=50000 --output_dir=outputs_50k/act_<skill>`; `python -m eval.skill_eval --act-root outputs_50k --tag act_50k` | `results/skill_eval_act_*50k.json` |
| Per-skill SmolVLA policy-only (step 5 000 of 20 000) | open_drawer **1/20** · pick_place_fork **0/20** (median 31.5 cm from zone) · pick_place_plate **0/20** (median 20.07 cm from zone) · pick_place_mug **0/20** (median 27.21 cm from zone) · handoff_spoon **0/20** (median 40.36 cm from zone) · hold_mug **3/20** · pour **0/20** | `python -m eval.skill_eval --kind smolvla --smolvla-path <Prashant-77/thali_smolvla> --tag smolvla_5k` | `results/skill_eval_smolvla_5k.json` |
...
| Target-volume pour | little (target 3): mean 4.0 spheres, within ±2 in **4/5**, reached 5/5 · normal (target 6): mean 6.0 spheres, within ±2 in **5/5**, reached 5/5 · full (target 12): mean 12.0 spheres, within ±2 in **5/5**, reached 5/5 | `python -m eval.pour_amount` | `results/pour_amount.json` |
