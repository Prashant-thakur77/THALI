...
|---|---|---|
| Task completion + bimanual (30) | 7-skill dinner task: drawer → fork → spoon **handed A→B via the table** → plate → **B holds mug while A pours** → mug set down | expert 5/10 test / 7/10 train; ACT policy-only pending, +retry pending, +fallback pending → `results/seeds.json` |
...
| OpenVINO / Core Ultra (20) | ACT → IR fp32/fp16 + NNCF int8, CPU + iGPU latency, 10-seed preservation per precision, VLM tok/s | `results/bench.json` (fp32/CPU 114.39 ms, fp32/GPU 555.63 ms, fp16/CPU 111.51 ms, fp16/GPU 556.94 ms, int8/CPU 36.95 ms, int8/GPU 421.94 ms), `results/preserve.json`, `results/ir_export.json`. **No NPU here**; static shapes ready for `-d NPU`. |
