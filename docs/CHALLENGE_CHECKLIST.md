...
|---|---|---|
| Task completion + bimanual (30) | 7-skill dinner task: drawer → fork → spoon **handed A→B via the table** → plate → **B holds mug while A pours** → mug set down | expert 8/10 test / 8/10 train; ACT policy-only 0/10, +retry 0/10, +fallback 0/10 → `results/seeds.json` |
