...
This is the plan for what comes next, ranked by
...
|---|---|---|---|
| **Clear the table** — *done* | reverse task: fork back to the drawer, spoon handed B→A and back, drawer closed (`put_in_drawer`, `close_drawer`; verifier orders close after stow) | done: `results/clear_table.json` **9/10** held-out (the miss is the set-up) | next: demos + ACT for the two skills; mug to the cabinet |
