...
| **Per-skill SmolVLA, policy only** (multi-task, language-conditioned; Kaggle T4 fine-tune in 4 500-step sessions — the newest checkpoint of the 20 000-step run, re-evaluated as sessions land) | step 14000: open_drawer **14/20** · pick_place_fork **0/20** (median 31.07 cm from zone) · pick_place_plate **2/20** (median 18.38 cm from zone) · pick_place_mug **0/20** (median 25.91 cm from zone) · handoff_spoon **0/20** (median 39.63 cm from zone) · hold_mug **8/20** · pour **0/20** | `results/skill_eval_smolvla_*.json` |
