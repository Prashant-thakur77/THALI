...
| | measured | evidence |
|---|---|---|
| **Full task, scripted expert** (drawer → fork → spoon handed A→B → plate → hold + pour → mug) | **8/10** held-out seeds · 8/10 train | `results/seeds_expert_*` |
...
| **Per-skill SmolVLA, policy only** (multi-task, language-conditioned; Kaggle T4 fine-tune in 4 500-step sessions — this row is the **step-5 000 checkpoint of 20 000**, re-evaluated as sessions land) | open_drawer **1/20** · pick_place_fork **0/20** (median 31.5 cm from zone) · pick_place_plate **0/20** (median 20.07 cm from zone) · pick_place_mug **0/20** (median 27.21 cm from zone) · handoff_spoon **0/20** (median 40.36 cm from zone) · hold_mug **3/20** · pour **0/20** | `results/skill_eval_smolvla_5k.json` |
...
| **Follow-ups and corrections** ("again", "a bit more", "no, the other side", "the other arm" resolved against the last executed step, then verified) | **19/20** corrections resolved to the expected plan; 14/16 resolved plans verifier-approved (the rest are refused with a reason, e.g. the other arm cannot reach) | `results/followups.json` |
...
| **Target-volume pour** ("a little" / normal / "fill it up" → 3 / 6 / 12 water spheres; the roll stops when the oracle counts the target) | little (target 3): mean 5.0 spheres, within ±2 in **4/6**, reached 6/6 · normal (target 6): mean 7.0 spheres, within ±2 in **5/6**, reached 6/6 · full (target 12): mean 13.3 spheres, within ±2 in **5/6**, reached 6/6 | `results/pour_amount.json` |
