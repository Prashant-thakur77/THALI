...
| **Per-skill ACT, policy only — 50k training steps** (same 150 episodes; the 12k-step rows above are the baseline) | pick_place_fork **16/20** (median 1.04 cm from zone) · pick_place_plate **15/20** (median 1.85 cm from zone) | `results/skill_eval_act_plate_50k.json`, `results/skill_eval_act_50k.json` |
