...
| **Clear the table** (reverse task on a set table: fork back to the drawer, spoon handed B→A and back, drawer closed — two new skills `put_in_drawer` / `close_drawer` through planner, verifier, queues and expert) | pending | `results/clear_table.json` |
